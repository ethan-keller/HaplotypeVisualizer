...
class GfaManager:
    ...
    def prepare_gfa(cls) -> None:
        try:
            ...
            if cls.recognize(gfa_file_path):
                gfa = Gfa.deserialize(from_file=FileManager.output_folder.joinpath(Path(f"{gfa_hash}.gfa.json")))
            else:
                gfa = Gfa.read_gfa_from_file(gfa_file_path)
                Gfa.serialize(gfa, out_file=FileManager.output_folder.joinpath(Path(f"{gfa_hash}.gfa.json")))
    ...
    def preprocess(cls) -> None:
        ...
        layout = Layout.compute_layout(file_path, FileManager.output_folder)
        LayoutManager.index = LayoutManager.get_index_from_layout(layout)
        gfa_hash = Gfa.get_gfa_hash(file_path)
        if gfa_hash:
            layout_path = KDTree.serialize(LayoutManager.index, FileManager.output_folder.joinpath(Path(f"{gfa_hash}.pickle")))
